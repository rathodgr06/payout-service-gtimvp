  ...
  try {
    const res = await axios.post(
      process.env.PAYOUT_MODE == "test"
        ? `${urls.mtn_mock.test_url}/disbursement/token/`
        : `${urls.mtn_mock.live_url}/disbursement/token/`,
      {},
      {
        headers: {
          Authorization:
            "Basic " + Buffer.from(`${mid}:${password}`).toString("base64"),
...
async function initiateTransfer(token, data, subscriptionKey) {
  ...
  try {
    ...
    console.log(
      "Using Base URL:",
      process.env.PAYOUT_MODE == "test"
        ? `${urls.mtn_mock.test_url}/disbursement/v1_0/transfer`
        : `${urls.mtn_mock.live_url}/disbursement/v1_0/transfer`
    );
    let headers = {
      Authorization: `Bearer ${token}`,
      "X-Reference-Id": referenceId,
      "X-Target-Environment":
        process.env.PAYOUT_MODE == "test" ? "sandbox" : "mtnliberia",
...
async function getTransferStatus(token, referenceId, subscriptionKey) {
  try {
    const res = await axios.get(
      process.env.PAYOUT_MODE == "test"
        ? `${urls.mtn_mock.test_url}/disbursement/v1_0/transfer/${referenceId}`
        : `${urls.mtn_mock.live_url}/disbursement/v1_0/transfer/${referenceId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          "X-Target-Environment":
